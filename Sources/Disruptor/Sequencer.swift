...
final class MultiProducerSequencer {
    ...
    func isAvailable(sequence: UInt64) -> Bool {
        ...
        let foo = UnsafeMutablePointer<Int>(mutating: availableBuffer).advanced(by: index)
        let val = volatile_load_int(UnsafeMutableRawPointer(foo))
        return val == flag
    }
