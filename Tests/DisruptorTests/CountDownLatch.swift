...
//
//  Modified by TupleStream OÜ to use Apple-provided swift-atomics primitives,
//  replacing the Objective-C bridge code
//
import Foundation
import Atomics
...
public class CountDownLatch {
    ...
    public init(count: Int) {
        ...
        initialCount = count
        conditionCount = ManagedAtomic<Int>(count)
    }
    ...
    public func countDown() {
        ...
        let newValue = conditionCount.wrappingDecrementThenLoad(ordering: .sequentiallyConsistent)
    ...
    public func await(timeout: TimeInterval) -> Bool {
        ...
        while conditionCount.load(ordering: .sequentiallyConsistent) > 0 {
    ...
    // Use `AtomicInt` to avoid contention during counting down and waiting. This allows the
    // lock to be only acquired at the time when the latch switches from closed to open.
    private let conditionCount: ManagedAtomic<Int>
}
